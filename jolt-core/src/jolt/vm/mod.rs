...
where
    ...
    ProofTranscript: Transcript,
{
    fn initialize(preprocessing: &JoltPreprocessing<C, F, PCS, ProofTranscript>) -> Self {
        // construct the object by using initialize method of all the field of JoltStuff
        Self {
    ...
    ProofTranscript: Transcript,
{
    ...
    fn prove(
        ...
        let mut jolt_polynomials: JoltStuff<MultilinearPolynomial<F>> = JoltPolynomials {
        ...
        for n in 0..no_of_shards {
            ...
            for shard in 0..shard_len {
                ...
                for i in 0..jolt_polynomials.r1cs.aux.relevant_y_chunks.len() {
                    assert_eq!(
                        streaming_r1cs_polynomials.shard.aux.relevant_y_chunks[i].get_coeff(shard),
                        jolt_polynomials.r1cs.aux.relevant_y_chunks[i]
                            .get_coeff(n * shard_len + shard)
                    );
                    assert_eq!(
                        streaming_r1cs_polynomials
                            .shard
                            .aux
                            .left_lookup_operand
                            .get_coeff(shard),
