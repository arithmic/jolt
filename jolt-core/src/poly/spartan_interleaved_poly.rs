...
impl<F: JoltField> SpartanInterleavedPolynomial<F> {
    ...
    pub fn first_sumcheck_round_<ProofTranscript: Transcript>(
        &mut self,
        eq_poly: &mut SplitEqPolynomial<F>,
        transcript: &mut ProofTranscript,
        r: &mut Vec<F>,
        ...
        let (evals, all_vecs): ((F, F, F), Vec<[[F; 4]; 4]>) = chunks
            .par_iter()
            .map(|chunk| {
                let mut eval_point_0 = F::zero();
                let mut eval_point_2 = F::zero();
                ...
                for sparse_block in chunk.chunk_by(|x, y| x.index / 6 == y.index / 6) {
                    ...
                    let eq_vec = [
                        E1_evals.0 * eq_poly.E2[prev_x2],
                        (E1_evals.1 + E1_evals.0) * two_inv * eq_poly.E2[prev_x2],
                        E1_evals.1 * eq_poly.E2[prev_x2],
                        E1_evals.2 * eq_poly.E2[prev_x2],
                    ];

                    let az_vec = [
                        F::from_i128(az.0),
                        F::from_i128(az.1),
                        F::from_i128(az_eval_2),
                        F::from_i128(az_eval_3),
                    ];
                    let bz_vec = [
                        F::from_i128(bz.0),
                        F::from_i128(bz.1),
                        F::from_i128(bz_eval_2),
                        F::from_i128(bz_eval_3),
                    ];
                    let cz_vec = [
                        F::from_i128(cz.0),
                        F::from_i128(cz.1),
                        F::from_i128(cz_eval_2),
                        F::from_i128(cz_eval_3),
                    ];
                    local_vecs.push([eq_vec, az_vec, bz_vec, cz_vec]);
                }
                ...
                ((eval_point_0, eval_point_2, eval_point_3), local_vecs)
            })
            .reduce(
                || ((F::zero(), F::zero(), F::zero()), Vec::new()),
                |(sum1, mut vec1), (sum2, vec2)| {
                    ((sum1.0 + sum2.0, sum1.1 + sum2.1, sum1.2 + sum2.2), {
                        vec1.extend(vec2);
                        vec1
                    })
                },
        ...
        let return_vec = all_vecs;
        let cubic_evals = [evals.0, /* 0 */ -evals.0, evals.1, evals.2];

        // for val in cubic_evals.iter(){
        //    println!("cubic_evals = {}", val);
        // }
