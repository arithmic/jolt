            ...
            move |shard_idx: usize,
                ...
                let az_bz_coeffs: Vec<SparseCoefficient<i128>> = (0..num_chunks)
                    ...
                    .flat_map_iter(|chunk_index| {
                        let mut local_az_bz_coeffs = Vec::with_capacity(
                            2 * chunk_size * padded_num_constraints
                        );
                        ...
                        for step_index in chunk_size * chunk_index..chunk_size * (chunk_index + 1) {
                            ...
                            for (constraint_index, constraint) in uniform_constraints
                                ...
                                let mut az_coeff = 0;
                                if !constraint.a.terms().is_empty() {
                                    az_coeff = constraint.a.evaluate_row(
                                        &input_polys_shard,
                                        step_index
                                    );
                                    if !az_coeff.is_zero() {
                                ...
                                let mut bz_coeff = 0;
                                if !constraint.b.terms().is_empty() {
                                    bz_coeff = constraint.b.evaluate_row(
                                        &input_polys_shard,
                                        step_index
                                    );
                                    if !bz_coeff.is_zero() {
                                        local_az_bz_coeffs.push(
                                            (global_index + 1, bz_coeff).into()
                                        );
                                    }
                                }
                            }
