...
use crate::{benches::bench::PCSType, field::JoltField, jolt::vm::rv32i_vm::ProofTranscript, poly::{commitment::commitment_scheme::CommitmentScheme, dense_mlpoly::DensePolynomial}, utils::transcript::{AppendToTranscript, Transcript}};

// #[derive(CanonicalSerialize, CanonicalDeserialize, Debug)]
pub struct KnowledgeProof<F: JoltField, PCS: CommitmentScheme<ProofTranscript, Field = F>> {
  alpha: PCS::Commitment,
  z1: F,
  z2: F,
}

impl<F: JoltField, PCS: CommitmentScheme<ProofTranscript, Field = F>> KnowledgeProof <F, PCS>{
  ...
  pub fn prove(
    ...
    random_tape: &mut RandomTape<F>,
    x: &F,
    r: &F,
  ) 
//   -> (KnowledgeProof<F, PCS>, PCS::Commitment)
   {
    // append protocol
...
pub struct RandomTape<F: JoltField>{
    tape: ProofTranscript,
    phantom_data: PhantomData<F>
}

impl<F: JoltField> RandomTape<F> {
    pub fn new(name: &'static [u8]) -> Self {
      let tape = {
        let mut prng = test_rng();
        let mut tape = Transcript::new(name);
        ProofTranscript::append_scalar(
          &mut tape,
        //   b"init_randomness",
          &F::random(&mut prng),
        );
    ...
    pub fn random_scalar(&mut self, label: &'static [u8]) -> F {
      ProofTranscript::challenge_scalar(&mut self.tape)
    }
  
    pub fn random_vector(&mut self, label: &'static [u8], len: usize) -> Vec<F> {
      ProofTranscript::challenge_vector(&mut self.tape, len)
    }
  }
