    ...
    ProofTranscript: Transcript,
{
        ...
        InstructionSet: JoltInstructionSet,
    {
        ...
        for idx in 0..val_prev.len() {
            ...
            let az_stream = temp_1[0].clone();
            let bz_stream = temp_1[1].clone();
            let cz_stream = temp_1[2].clone();
            for j in 0..4 {
                // println!("az_pre[{}] = {}", j, az[j]);
                // println!("az_stream[{}] = {}", j, az_stream[j]);
                assert_eq!(az[j], az_stream[j], "failed az at idx = {idx} j = {}", j);
                assert_eq!(cz[j], cz_stream[j], "failed cz at idx = {idx} j = {}", j);
                assert_eq!(bz[j], bz_stream[j], "failed bz at idx = {idx} j = {}", j);
            }
        }
