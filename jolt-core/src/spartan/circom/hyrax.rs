use std::fmt;
use ark_grumpkin::{Fr as Scalar, Fq as Fp, Projective, Affine};
...
pub fn hyrax_gens_to_circom(gens: &PedersenGenerators<Projective>,    proof: &SpartanProof<Scalar, HyraxScheme<Projective, GrumpkinPoseidonTranscript<Fp>>, GrumpkinPoseidonTranscript<Fp>>) -> HyraxGensCircom {
...
pub fn hyrax_commitment_to_circom(commit: &HyraxCommitment<Projective>)-> HyraxCommitmentCircom{
    HyraxCommitmentCircom(commit.row_commitments.iter().map(|g| G1AffineCircom::from_g1(
        &Projective::from(*g).into_affine()
     )).collect())
}

pub fn hyrax_eval_proof_to_circom(eval_proof: &HyraxOpeningProof<Projective, GrumpkinPoseidonTranscript<Fp>>)-> HyraxEvalProofCircom{
