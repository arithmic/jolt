...
use super::{
    joltproof_bytecode_proof::{convert_multiset_hashes_to_circom, MultiSethashesCircom},
    struct_fq::FqCircom,
    sum_check_gkr::{
        convert_from_batched_GKRProof_to_circom, convert_sum_check_proof_to_circom,
        BatchedGrandProductProofCircom, SumcheckInstanceProofCircom,
    },
};
use crate::lasso::memory_checking::StructuredPolynomialData;
use crate::{
    jolt::vm::read_write_memory::ReadWriteMemoryProof, poly::commitment::hyperkzg::HyperKZG,
...
pub fn convert_from_read_write_mem_proof_to_circom(
    rw_mem_proof: ReadWriteMemoryProof<
        Scalar,
    ...
    openings.push(FqCircom(rw_openings.a_ram));
    openings.push(FqCircom(rw_openings.v_read_rd));
    openings.push(FqCircom(rw_openings.v_read_rs1));
    openings.push(FqCircom(rw_openings.v_read_rs2));
    openings.push(FqCircom(rw_openings.v_read_ram));
    openings.push(FqCircom(rw_openings.v_write_rd));
    openings.push(FqCircom(rw_openings.v_write_ram));
    openings.push(FqCircom(rw_openings.v_final));
    openings.push(FqCircom(rw_openings.t_read_rd));
    openings.push(FqCircom(rw_openings.t_read_rs1));
    ...
    exogenous_openings.push(FqCircom(exogenous_openings_from_rust.a_rd));
    exogenous_openings.push(FqCircom(exogenous_openings_from_rust.a_rs1));
    exogenous_openings.push(FqCircom(exogenous_openings_from_rust.a_rs2));

    let mem_checking_proof = ReadWriteMemoryCheckingProofCircom {
        multiset_hashes: convert_multiset_hashes_to_circom(
            &rw_mem_proof.memory_checking_proof.multiset_hashes,
        ),
        read_write_grand_product: convert_from_batched_GKRProof_to_circom(
            &rw_mem_proof.memory_checking_proof.read_write_grand_product,
        ),
        init_final_grand_product: convert_from_batched_GKRProof_to_circom(
            &rw_mem_proof.memory_checking_proof.init_final_grand_product,
        ),
        openings,
        exogenous_openings,
    };
    ...
    for opening in ts_exo_openings {
        exo_openings.push(FqCircom(opening.clone()));
    }

    let ts_validity_proof = TimestampValidityProofCircom {
        multiset_hashes: convert_multiset_hashes_to_circom(
            &rw_mem_proof.timestamp_validity_proof.multiset_hashes,
        ),
        openings: TimestampRangeCheckOpenings {
            read_cts_read_timestamp: openings[0..MEMORY_OPS_PER_INSTRUCTION].to_vec(),
            read_cts_global_minus_read: openings
                [MEMORY_OPS_PER_INSTRUCTION..2 * MEMORY_OPS_PER_INSTRUCTION]
                .to_vec(),
            final_cts_read_timestamp: openings
                [2 * MEMORY_OPS_PER_INSTRUCTION..3 * MEMORY_OPS_PER_INSTRUCTION]
                .to_vec(),
        ...
        batched_grand_product: convert_from_batched_GKRProof_to_circom(
            &rw_mem_proof.timestamp_validity_proof.batched_grand_product,
        ),
    };

    let ouput_sum_check_proof: OutputSumcheckProofCircom = OutputSumcheckProofCircom {
        sumcheck_proof: convert_sum_check_proof_to_circom(
            &rw_mem_proof.output_proof.sumcheck_proof,
        ),
