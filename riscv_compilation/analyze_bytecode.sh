#!/bin/bash
# If there is some error, refer to readme.md
...
echo "$DISASM" > disasm.txt

echo ""
echo "------ Instruction Count ------"

TOTAL=$(echo "$DISASM" | grep -E '^\s*[0-9a-f]+:\s+[0-9a-f]+' | wc -l)

# Instruction categories
count_instr() {
    echo "$DISASM" | grep -E "\s($1)\s" | wc -l
}
...
COMPRESSED=$(echo "$DISASM" | grep -E '^\s*[0-9a-f]+:\s+[0-9a-f]{1,4}\s+' | wc -l)
